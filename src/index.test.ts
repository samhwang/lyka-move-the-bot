...
describe('Move Robot Tests', () => {
  describe('Part 1', () => {
    it('Should stay still if the command is empty', () => {
      ...
      const newLocation = execute({ robotPosition: start }, '');
      expect(newLocation).toEqual({ robotPosition: start });
    });
    ...
    it.each<[string, Coordinates]>([
      ...
      const newLocation = execute({ robotPosition: start }, command);
      expect(newLocation).toEqual({ robotPosition: start });
    });
    ...
    it.each<[string, string, Coordinates]>([
      ...
      const newLocation = execute({ robotPosition: start }, command);
      expect(newLocation).toEqual({ robotPosition: end });
    });
